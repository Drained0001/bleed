...
class view(View):
    ...
    async def back(self, button: Button, interaction: Interaction):
        if interaction.user == self.ctx.author:
            ...
            embed = discord.Embed(title="List of roles", description=text, color=color)
...
class roles(commands.Cog):
    ...
    @commands.command(description="View all roles in the server")
    async def roles(self, ctx):
        ...
        bots = [m for m in ctx.guild.roles][::-1]
        bots = [f"`{bots.index(m)+1}` {m.mention}" for m in bots]

        if len(bots) < 10:
            embed = discord.Embed(title="List of roles", description='\n'.join(bots), color=color)
