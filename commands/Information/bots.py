...
class view(View):
    ...
    async def back(self, button: Button, interaction: Interaction):
        if interaction.user == self.ctx.author:
            ...
            embed = discord.Embed(title="List of bots", description=text, color=color)
...
class bots(commands.Cog):
    ...
    @commands.command(description="View all bots in the server")
    async def bots(self, ctx):
        ...
        bots = [f"`{bots.index(m)+1}` **{m}**" for m in bots]

        if len(bots) < 10:
            embed = discord.Embed(title="List of bots", description='\n'.join(bots), color=color)
