...
class view(View):
    ...
    async def back(self, button: Button, interaction: Interaction):
        if interaction.user == self.ctx.author:
            ...
            embed = discord.Embed(title="List of boosters", description=text, color=color)
...
class boosters(commands.Cog):
    ...
    @commands.command(description="View all recent server boosters")
    async def boosters(self, ctx):
        ...
        bots = sorted(ctx.guild.premium_subscribers, key=lambda d: datetime.datetime.timestamp(d.premium_since))[::-1]
        bots = [f"`{bots.index(m)+1}` {m.mention} boosted <t:{(round(datetime.datetime.timestamp(m.premium_since)))}:R>" for m in bots]

        if len(bots) < 10:
            embed = discord.Embed(title="List of boosters", description='\n'.join(bots), color=color)
