...
class view(View):
    ...
    async def back(self, button: Button, interaction: Interaction):
        if interaction.user == self.ctx.author:
            ...
            embed = discord.Embed(title="List of emojis", description=text, color=color)
...
class emojis(commands.Cog):
    ...
    @commands.command(description="View all emotes in the server")
    async def emojis(self, ctx):
        ...
        bots = [m for m in ctx.guild.emojis]
        bots = [f"{m} [{m.name}]({m.url})" for m in bots]

        if len(bots) < 10:
            embed = discord.Embed(title="List of emojis", description='\n'.join(bots), color=color)
